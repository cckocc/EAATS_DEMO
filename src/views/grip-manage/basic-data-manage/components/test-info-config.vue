  ...
  <div>
    ...
    <div class="main-body">
      <div class="main-body-top">
        <el-form
          ...
          inline
        >
          <el-form-item label="测试信息名称" prop="testInformationName">
            <el-input v-model.trim="mainObj.searchForm.testInformationName" placeholder="请输入" clearable />
          </el-form-item>
    ...
    >
      ...
      >
        <el-form-item label="测试信息名称" prop="testInformationName">
          <el-input
            v-model="dialogObj.form.testInformationName"
            v-limitBytes="300"
            placeholder="请输入"
            clearable
          />
        ...
        <el-form-item label="测试信息内容" prop="testInformationContent">
          <Tinymce v-if="dialogObj.type !== 'detail'" ref="testInformationContent" v-model="dialogObj.form.testInformationContent" />
          <div v-else class="rich-text">
            <div v-html="dialogObj.form.testInformationContent" />
          </div>
          <!-- <el-input
            v-model="dialogObj.form.testInformationContent"
            type="textarea"
            placeholder="请输入"
  ...
  methods: {
    ...
    fetchData() {
      ...
      const params = {
        ...
        testInformationName: this.mainObj.searchForm.testInformationName,
      ...
      basicDataConfigApi.getTestInfoConfigList(params).then(res => {
        if (res.code === 'ACK') {
    ...
    getTreeNode() {
      ...
      if (this.dialogObj.type !== 'add') {
        params.id = this.dialogObj.form.testInformationId
      }
    ...
    onOperate(type, row) {
      ...
      if (type === 'add') {
        ...
        this.$nextTick(() => {
          try {
            this.$refs.testInformationContent.setContent('')
          } catch (e) {
            console.log(e)
          }
        ...
        this.dialogObj.tempTreeIds = []
        const params = {
          testInformationId: row.testInformationId
        }
        basicDataConfigApi.getTestInfoConfigDetail(params).then(res => {
          this.dialogObj.form.ids = res.data.checkedMenuConfigNodeList
          this.dialogObj.form.testInformationId = res.data.testInformationConf.testInformationId
          this.dialogObj.form.testInformationName = res.data.testInformationConf.testInformationName
          this.dialogObj.form.testInformationContent = res.data.testInformationConf.testInformationContent
          this.$nextTick(() => {
            try {
              this.$refs.testInformationContent.setContent(res.data.testInformationConf.testInformationContent)
            } catch (e) {
              console.log(e)
            }
            this.$refs.dialogForm.clearValidate()
        ...
        this.$confirm('删除之后所配置的接口在服务开通申请中将不能自动回显测试信息，需场务用户手动输入，确认删除？', '提示', {
          ...
          basicDataConfigApi.deleteTestInfoConfig(params).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功!'
            })
        ...
        this.$refs.dialogForm.validate((valid) => {
          if (valid) {
            ...
            const params = {
              testInformationName: this.dialogObj.form.testInformationName,
              testInformationContent: this.dialogObj.form.testInformationContent,
              checkedMenuConfigNodeList: this.dialogObj.form.ids,
              institutionType: this.institutionType
            }
            if (this.dialogObj.type === 'edit') {
              params.testInformationId = this.dialogObj.form.testInformationId
            }
            console.log(params)
            basicDataConfigApi.saveTestInfoConfig(params, this.dialogObj.type).then(res => {
              if (res.code === 'ACK') {
                this.$message({
                  type: 'success',
                  message: res.message
