  ...
  <div>
    ...
    <div class="main-body">
      <div class="main-body-top">
        <el-form
          ...
          inline
        >
          <el-form-item label="防火墙名称" prop="firewallName">
            <el-input v-model.trim="mainObj.searchForm.firewallName" placeholder="请输入" clearable />
          </el-form-item>
          ...
          <el-form-item label="更新人" prop="lastMntUserName">
    ...
    >
      ...
      >
        <el-form-item label="防火墙名称" prop="firewallName">
          <el-input
            v-model="dialogObj.form.firewallName"
            v-limitBytes="300"
            placeholder="请输入"
            clearable
          />
        ...
        <el-form-item label="防火墙信息内容" required>
          <el-table
            :data="dialogObj.form.firewallInformationRefs"
            fit
            stripe
            border
            highlight-current-row
          >
            ...
            >
              <template slot-scope="scope">
                <el-form-item
                  ...
                  label=""
                  label-width="0"
                  :prop="'firewallInformationRefs.' + scope.$index + '.transactionCenterServerAddress'"
                  :rules="dialogObj.rules.blurRequired"
                >
                  <el-input
                    v-model.trim="scope.row.transactionCenterServerAddress"
                    v-limitBytes="300"
                    placeholder="请输入"
                    clearable
                  />
                </el-form-item>
                <span v-else>{{ scope.row.transactionCenterServerAddress }}</span>
              </template>
            </el-table-column>
            <el-table-column
              label="服务端口"
            ...
            >
              <template slot-scope="scope">
                <el-form-item
                  ...
                  label=""
                  label-width="0"
                  :prop="'firewallInformationRefs.' + scope.$index + '.servicePort'"
                  :rules="dialogObj.rules.blurRequired"
                >
                  <el-input
            ...
            >
              <template slot-scope="scope">
                <el-form-item
                  ...
                  label=""
                  label-width="0"
                  :prop="'firewallInformationRefs.' + scope.$index + '.remark'"
                  :rules="dialogObj.rules.blurRequired"
                >
                  <el-input
            ...
            >
              <template slot-scope="scope">
                <el-button type="text" :disabled="dialogObj.form.firewallInformationRefs.length < 2" @click="deleteFirewall(scope.row, scope.$index)"><svg-icon icon-class="delete" /></el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-button
...
const fireWallObj = {
  transactionCenterServerAddress: '',
  servicePort: '',
  remark: ''
}
const defaultDialogForm = {
  ids: [],
  firewallName: '',
  firewallInformationRefs: [fireWallObj]
}
export default {
  data() {
    ...
    return {
      ...
      mainObj: {
        ...
        currentPage: 1,
        total: 0,
        searchForm: Object.assign({}, defaultDialogForm)
      },
      dialogObj: {
        visible: false,
        title: '',
        ...
        rules: {
          ...
          blurRequired: [{ required: true, message: '不能为空', trigger: 'blur' }],
          ids: [
  ...
  methods: {
    ...
    fetchData() {
      ...
      const params = {
        ...
        firewallName: this.mainObj.searchForm.firewallName,
      ...
      basicDataConfigApi.getFirewallConfigList(params).then(res => {
        if (res.code === 'ACK') {
    ...
    getTreeNode() {
      ...
      if (this.dialogObj.type !== 'add') {
        params.id = this.dialogObj.form.firewallId
      }
    ...
    onOperate(type, row) {
      ...
      if (type === 'add') {
        ...
        this.dialogObj.form = Object.assign({}, JSON.parse(JSON.stringify(defaultDialogForm)), row)
        this.dialogObj.tempTreeIds = []
        const params = {
          firewallId: row.firewallId
        }
        basicDataConfigApi.getFirewallConfigDetail(params).then(res => {
          this.dialogObj.form.ids = res.data.checkedMenuConfigNodeList
          this.dialogObj.form.firewallInformationRefs = res.data.firewallEditVo.firewallInformationRefs
          this.$nextTick(() => {
            this.$refs.dialogForm.clearValidate()
          })
        })
        ...
        this.$confirm('删除之后所配置的接口在接口上线申请中将不能自动回显防火墙，需场务用户手动输入，确认删除？', '提示', {
          ...
          basicDataConfigApi.deleteFirewallConfig(params).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功!'
            })
        ...
        this.$refs.dialogForm.validate((valid) => {
          if (valid) {
            ...
            const params = {
              firewallName: this.dialogObj.form.firewallName,
              firewallInformationRefs: this.dialogObj.form.firewallInformationRefs,
              checkedMenuConfigNodeList: this.dialogObj.form.ids,
              institutionType: this.institutionType
            }
            if (this.dialogObj.type === 'edit') {
              params.firewallId = this.dialogObj.form.firewallId
            }
            console.log(params)
            basicDataConfigApi.saveFirewallConfig(params, this.dialogObj.type).then(res => {
              if (res.code === 'ACK') {
                this.$message({
                  type: 'success',
                  message: res.message
    ...
    addFirewall() {
      this.dialogObj.form.firewallInformationRefs.push({
        transactionCenterServerAddress: '',
        servicePort: '',
        remark: ''
      })
    },
    deleteFirewall(row, index) {
      this.dialogObj.form.firewallInformationRefs = this.dialogObj.form.firewallInformationRefs.filter((e, i) => i !== index)
    }
  }
}
</script>
