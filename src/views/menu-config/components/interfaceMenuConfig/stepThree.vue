  ...
  <div>
    ...
    <div>
      ...
      >
        <template v-slot:node="{ node }">
          <div v-if="node.nodeType === 'extra'" class="extra-container" @dblclick="dblclick(node)">
            <div v-if="node.extraType === 'serviceOpening'">
              <el-input v-if="node.showSelect" v-model="node.openDimensionName" v-limitBytes="300" type="textarea" autosize @change="val => onChange(val, node, 'openDimensionName')" />
              <span v-else>{{ node.openDimensionName }}</span>
            </div>
            <div v-if="node.extraType === 'serviceOnline'">
              <el-input v-if="node.showSelect" v-model="node.onlineDimensionName" v-limitBytes="300" type="textarea" autosize @change="val => onChange(val, node, 'onlineDimensionName')" />
              <span v-else>{{ node.onlineDimensionName }}</span>
            </div>
          </div>
        </template>
        <!-- <template v-slot:title="{ node }">
          <template v-if="node.nodeType === 'extra'">
            <div v-if="node.extraType === 'serviceOpening'">
              <el-button class="extra-custom-btn" type="primary">{{ node.name }}</el-button>
            </div>
            <div v-if="node.extraType === 'serviceOnline'">
              <el-button class="extra-custom-btn" type="primary">{{ node.name }}</el-button>
            </div>
      ...
      >
        <template v-slot:node="{ node }">
          <template v-if="node.nodeType === 'extra'">
            <span v-if="node.extraType === 'serviceOpening'">{{ node.openDimensionName }}</span>
            <span v-if="node.extraType === 'serviceOnline'">{{ node.onlineDimensionName }}</span>
          </template>
        </template>
      </tableTree>
    </div>
