    ...
    <div>
      ...
      >
        <!-- <template v-slot:node="{ node }">
          ...
          <i v-if="node.guideHideDownloadFlag === 'YES'" style="position: absolute; top: 2px; right: 2px;" class="el-icon-view" />
        </template> -->
        <template v-slot:node="{ node }">
          <div v-if="node.guideHideDownloadFlag === 'YES'" class="hide-flag">
  ...
  methods: {
    ...
    onOperate(type) {
      ...
      if (type === 'hide') {
        ...
        // const waitNodeList = checkedNodelist.filter(e => e.guideHideDownloadFlag === 'NO')
        ...
        const hasFlag = checkedNodelist.some(e => e.guideHideDownloadFlag === 'YES')
        if (hasFlag) {
          this.$message({
            type: 'info',
          ...
          this.$refs.tableTree.changeNodeValue(this.$refs.tableTree.getCheckedNodeList().map(e => e.id), 'guideHideDownloadFlag', 'YES')
        }
        // const params = {
        //   recordId: this.currentRow.id,
        //   guideHideDownloadFlag: '1',
        ...
        const checkedNodelist = this.$refs.tableTree.getCheckedNodeList()
        // console.log(checkedNodelist)
        // const waitNodeList = checkedNodelist.filter(e => e.guideHideDownloadFlag === 'YES')
        if (checkedNodelist.length === 0) {
          this.$message({
            type: 'info',
        ...
        const hasFlag = checkedNodelist.some(e => e.guideHideDownloadFlag === 'NO')
        if (hasFlag) {
          this.$message({
            type: 'info',
          ...
          this.$refs.tableTree.changeNodeValue(this.$refs.tableTree.getCheckedNodeList().map(e => e.id), 'guideHideDownloadFlag', 'NO')
        }
        // const params = {
        //   recordId: this.currentRow.id,
        //   guideHideDownloadFlag: '0',
        //   checkedMenuConfigNodeIdList: checkedNodelist.map(e => e.id)
        // }
        // menuConfigApi.toggleMenuConfigNode(params).then(res => {
        //   if (res.code === 'ACK') {
