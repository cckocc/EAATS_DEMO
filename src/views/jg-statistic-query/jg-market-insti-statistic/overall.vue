...
// import { getAllCheckedNodeList } from '@/components/TableTree/utils/index.js'
...
export default {
  ...
  methods: {
    ...
    async onQuery() {
      ...
      const res = await reqQueryMarketOverall(params)
      if (res.code === 'ACK') {
        this.loading = false
    ...
    async outputExcel() {
      ...
      reqExportJgMarketExcel(params).then(res => {
        if (res.status === 200) {
          const name = '机构端市场机构统计总体查询' + '.xlsx'
          loading.close()
