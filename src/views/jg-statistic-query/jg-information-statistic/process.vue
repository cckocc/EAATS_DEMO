...
export default {
  ...
  data() {
    return {
      ...
      columnCheckedList: ['isvFlag', '产品类型', '数据内容', 'onlineIsvAgencyName', 'api', 'onlineOverTime',
      ...
      overallQueryList: [
        ...
        { label: 'c(上线)', value: 'onlineIsvAgencyName', width: '170' },
  ...
  methods: {
    ...
    async onQuery() {
      ...
      const res = await reqQueryProcessAgencyInsti(params)
      if (res.code === 'ACK') {
    ...
    quicklyOutput() {
      ...
      reqExportProcessAgencyInsti(params).then(res => {
        if (res.status === 200) {
          const name = '机构端信息商统计流程查询' + '.xlsx'
