        ...
        <div class="table">
          ...
          >
            <el-table-column
              ...
              width="150"
            />
            <el-table-column align="center" :width="800+columnCheckedList.length*200">
              <template #header>
                <SearhTreeSlot
                  :origin-data="[]"
...
export default {
  ...
  data() {
    return {
      ...
      columnCheckedList: ['api', '产品类型', '数据内容', 'onlineOverTime',
  ...
  methods: {
    ...
    async onQuery() {
      ...
      const res = await reqQueryOnlineAgencyInsti(params)
    ...
    quicklyOutput() {
      ...
      reqExportOnlineAgencyInsti(params).then(res => {
        if (res.status === 200) {
          const name = '机构端信息商统计上线通过查询' + '.xlsx'
          loading.close()
