    ...
    <div class="main-body">
      <div class="main-body-top">
        ...
        >
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="机构名称">
                <el-input
                  v-model="processForm.agencyName"
                  class="fixed_width"
                  placeholder="请输入机构名称"
                  clearable
                />
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="是否是ISV">
                <el-select
                  v-model="processForm.displayIsvFlag"
                  placeholder="--全部--"
                  style="width: 100%;"
                  clearable
                >
                  <el-option
                    v-for="item in isvAccoutList"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
      ...
      <div class="main-body-bottom">
        ...
        <div class="table">
          ...
          >
            ...
            />
            <el-table-column
              v-if="columnCheckedList.indexOf('isvFlag') >= 0"
              prop="isvFlag"
              label="是否是ISV"
              align="center"
              width="150"
...
export default {
  ...
  data() {
    return {
      ...
      columnCheckedList: ['agencyName', 'isvFlag', '产品类型', '数据内容', 'onlineIsvAgencyName', 'api', 'onlineOverTime',
  ...
  methods: {
    ...
    async onQuery() {
      ...
      const params = {
        agencyId: '',
        agencyName: this.processForm.agencyName,
      ...
      const res = await reqQueryProcessInsti(params)
      if (res.code === 'ACK') {
            ...
            item.chidrenList.length === 0
              ...
              if (item.extraInfoList && item.extraInfoList.length > 0) {
                item.extraInfoList.forEach((extra) => {
                  ...
                  acceptanceTestIsvAgencyNameList.push(extra.acceptanceTestIsvAgencyName)
    ...
    quicklyOutput() {
      ...
      reqExportProcessInsti(params).then(res => {
        if (res.status === 200) {
          const name = '场务端信息商统计流程查询' + '.xlsx'
    ...
    onChangeColumnChecked() {
      console.log(this.columnCheckedList)
      console.log(this.tableDataList)
      // this.tableDataList.forEach(tableData => {
      //   // 过滤不要的字段
      //   tableData.extraColumnObj.list.filter(item => {
      //     if (this.columnCheckedList.indexOf(item.thName) >= 0) {
      //       return item
      //     }
      //   })
      // })
