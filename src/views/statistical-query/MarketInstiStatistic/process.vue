...
export default {
  ...
  data() {
    return {
      ...
      columnCheckedList: ['agencyName', 'isvFlag', '接口产品', '服务内容', '接口用途', '接口服务', '市场', 'onlineIsvAgencyName', 'api', 'onlineOverTime',
  ...
  methods: {
    ...
    async onQuery() {
      ...
      const res = await reqQueryProcess(params)
      if (res.code === 'ACK') {
        this.tableDataList = res.data.bodyData.list
        this.tableDataList && this.tableDataList.forEach((e) => {
    ...
    quicklyOutput() {
      ...
      reqExportProcess(params).then(res => {
        if (res.status === 200) {
          loading.close()
          const name = '场务端市场机构统计流程查询' + '.xlsx'
