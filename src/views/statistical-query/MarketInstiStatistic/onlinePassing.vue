            ...
            <el-table-column align="center" :min-width="columnCheckedList.length*200">
              ...
              <template slot-scope="scope">
                <SearhTreeSlot
                  ...
                  extra-column
                >
                <!-- <span v-for="item in scope.row.extraColumnObj.columns" :key="item.rowspan">
                  <span v-if="item.name === '上线服务单号'" @click="jump()">
                    上线服务单号
                  </span>
                </span> -->
                </SearhTreeSlot>
...
export default {
  ...
  data() {
    return {
      ...
      columnCheckedList: ['agencyName', 'api', '接口产品', '服务内容', '接口用途', '接口服务', '市场', 'onlineOverTime',
  ...
  methods: {
    ...
    async onQuery() {
      ...
      const res = await reqQueryOnlineMarket(params)
      if (res.code === 'ACK') {
    ...
    quicklyOutput() {
      ...
      reqExportOnlineMarket(params).then(res => {
        if (res.status === 200) {
          const name = '场务端市场机构统计上线通过查询' + '.xlsx'
