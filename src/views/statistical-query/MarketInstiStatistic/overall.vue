      ...
      <div class="main-body-top">
        ...
        >
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="机构名称" prop="agencyName">
                <el-input
                  v-model="overallForm.agencyName"
                  class="fixed_width"
                  placeholder="请输入机构名称"
                  clearable
                />
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="接口用途" prop="interfacePurpose">
                <el-input
                  v-model="overallForm.interfacePurpose"
                  class="fixed_width"
                  placeholder="请输入接口用途"
                  clearable
                />
            ...
            <el-col :span="8">
              <el-form-item label="接口产品" prop="interfaceProduct">
                <el-input
                  v-model="overallForm.interfaceProduct"
                  class="fixed_width"
                  placeholder="请输入接口产品"
                  clearable
                />
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="服务内容" prop="serviceContent">
                <el-input
                  v-model="overallForm.serviceContent"
                  class="fixed_width"
                  placeholder="请输入服务内容"
                  clearable
                />
  ...
  data() {
    return {
      ...
      overallForm: {
        ...
        procOverTime: null, // 审核结束时间
        api: '', // api账号
        market: '', // 市场

        interfacePurpose: '', // 接口用途
        interfaceProduct: '', // 接口产品
        serviceContent: '' // 服务内容
      },
      searchForm: {
        applyNo: '', // 服务单号
      ...
      overallQueryList: [
        ...
        { label: '市场', value: 'market', width: '170' },
        { label: '接口用途', value: 'interfacePurpose', width: '170' },
        { label: '接口产品', value: 'interfaceProduct', width: '170' },
        { label: '服务内容', value: 'serviceContent', width: '170' }
      ],
  ...
  methods: {
    ...
    async onQuery() {
      ...
      const params = {
        ...
        agencyName: this.searchForm.agencyName,
        ...
        applyStartTime: this.searchForm.applyTime ? this.searchForm.applyTime[0] : '',
        applyEndTime: this.searchForm.applyTime ? this.searchForm.applyTime[1] : '',
        procOverStartTime: this.searchForm.procOverTime ? this.searchForm.procOverTime[0] : '',
        procOverEndTime: this.searchForm.procOverTime ? this.searchForm.procOverTime[1] : '',
        api: this.searchForm.api,
        market: this.searchForm.market,
        interfacePurpose: this.searchForm.interfacePurpose,
        interfaceProduct: this.searchForm.interfaceProduct,
        serviceContent: this.searchForm.serviceContent,
        // 是否需要分页？
        page: this.currentPage,
        pageSize: this.pageSize,
        leafNodeIdList: this.leafNodeIdList
      }
      const res = await reqQueryOverall(params)
      if (res.code === 'ACK') {
        this.loading = false
        // console.log('11112212', this.loading)
    ...
    quicklyOutput() {
      ...
      reqExportMarketExcel(params).then(res => {
        if (res.status === 200) {
          loading.close()
          const name = '场务端市场机构统计总体查询' + '.xlsx'
    ...
    onChangeColumnChecked(val) {
      ...
      if (val.includes('interfacePurpose')) {
        this.columnNo = this.columnNo + 1
      }
      if (val.includes('interfaceProduct')) {
        this.columnNo = this.columnNo + 1
      }
      if (val.includes('serviceContent')) {
        this.columnNo = this.columnNo + 1
      }
    },
