  ...
  methods: {
    confirmBut() {
      const selectConst = `
        ...
        tagName="true"
        onselectstart="false"
        ><custom style="color: #2C76D1;
        background-color: #EDF4FF !important;
        user-select: none;
        padding:3px 7px;display:
        inline-block;" custom="custom">${this.selectElements}</custom>
        </span>`
      this.dataContent = selectConst
      this.$refs.updateTiny.insertElement()
      this.$refs.insertDialog.visibleInsert = false
    },
    insertElement() {
      reqInsertElements({ ...this.lookType.data.currentNode })
        .then((res) => {
          const { code, data } = res
          if (code === 'ACK') {
    ...
    test() {
      // const dataContent = "<div style='color: red'>内容信息</div>"
