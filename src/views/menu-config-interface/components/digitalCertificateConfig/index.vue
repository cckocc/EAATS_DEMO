    ...
    <div class="main-body-bottom">
      <div class="main-body-bottom-btn">
        <div class="main-body-bottom-btn-left">
          <el-button type="primary" size="small" @click="showUploadDialog('add')">新增数字证书</el-button>
        </div>
      </div>
      <el-table
        :data="mainObj.list"
...
export default {
  ...
  methods: {
    ...
    onOperate(type, row) {
      ...
      if (type === 'edit') {
        ...
        if (this.uploadDialogObj.type === 'add') {
          ...
          menuConfigApi.addDigitalCertificateConfig(form).then(res => {
            if (res.code === 'ACK') {
              this.$message({
                message: `数字证书新增成功`,
                type: 'success'
          ...
          menuConfigApi.updateDigitalCertificateConfig(form).then(res => {
            if (res.code === 'ACK') {
              this.$message({
                message: `数字证书更新成功`,
                type: 'success'
    ...
    getVersionRecordList() {
      ...
      menuConfigApi.getDigitalCertificateConfigHistory(params).then(res => {
        if (res.code === 'ACK') {
    ...
    onOperateVersionRecord(type, row) {
      ...
      if (type === 'download') {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          menuConfigApi.deleteDigitalCertificateConfig(params).then(res => {
            if (res.code === 'ACK') {
              this.$message({
                message: `文件删除成功`,
                type: 'success'
    ...
    showUploadDialog(type, row) {
      ...
      if (type === 'add') {
        this.uploadDialogObj.title = `新增数字证书`
      } else if (type === 'edit') {
        this.uploadDialogObj.row = row
        this.uploadDialogObj.title = `更新数字证书-${row.fileName}`
      }
    },
    closeUploadDialog() {
