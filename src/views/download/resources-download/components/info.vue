            ...
            >
              <div class="tag">
                <el-select
                  ...
                  v-model="tag.id"
                  class="number"
                  @change="val => changeId(val, tag, key, item.waitOnlineResource)"
                >
                  <el-option
...
export default {
  ...
  methods: {
    ...
    changeId(val, tag, key, wait) {
      console.log(val, tag, key, wait)
      if (tag.fileType.code === 'DEVELOP_GUIDE') {
        tag.versionOptions.forEach((item) => {
          if (item.devGuideId === val) {
